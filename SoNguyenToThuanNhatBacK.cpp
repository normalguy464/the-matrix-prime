...
int main(){
	...
	sort(a, a+n); //sap xep cac phan tu trong mang theo thu tu tang dan, coi phan tu nay la phan tu dau tien.
	...
	while(check){
		...
		for(int i=1; i<n; i++){
			...
			if(t>res1) res1 = t; //goi res1 la chi so lon nhat sao cho a[i+1]>a[i]
		}
		int res2 = INT_MAX;
		...
		for(int i=1; i<=n; i++){
			if(a[i]==res2){
				int temp = a[res1-1]; // doi cho 2 phan tu de thuc hien sap xep
				a[res1-1] = a[i];
				a[i] = temp;
			}
		}
